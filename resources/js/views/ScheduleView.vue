...
const blankForm = () => ({ title: '', type: 'coletiva', status: 'agendado', startsAt: dateTimeFor(selectedDate.value, 9), endsAt: dateTimeFor(selectedDate.value, 10), repeatFrequency: 'none', repeatUntil: '', instructorId: '', modality: 'presencial', location: '', address: '', capacity: null, notes: '', participantIds: [] });
...
function formatDate(date, format) { const options = format === 'EEEE' ? { weekday: 'long' } : format === 'EEE' ? { weekday: 'short' } : format === 'd' ? { day: 'numeric' } : { day: 'numeric', month: format.includes('MMMM') ? 'long' : 'short', year: format.includes('yyyy') ? 'numeric' : undefined }; return new Intl.DateTimeFormat('pt-BR', options).format(new Date(date)); }
